...
export async function rotateRefreshToken(compositeToken: string) {
  const [tokenId, rawSecret] = compositeToken.split(".");
  if (!tokenId || !rawSecret) throw new Error("Malformed refresh token");
  const record = await prisma.refreshToken.findUnique({ where: { id: tokenId } });
  if (!record || record.revoked || record.expiresAt < new Date()) throw new Error("Invalid refresh token");
